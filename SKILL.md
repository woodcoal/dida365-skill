...
所有命令均采用 `python3 index.py <category> <subcommand> [args]` 格式。支持 `-h` 查看详细帮助。
...
│  ├─ 删除任务 ──────────→ task delete {projectId} {taskId}
...
### 精确核对
使用 `python3 index.py task get {projectId} {taskId}` 查看最完整的任务详情（包括子项状态、时区、提醒等）。
